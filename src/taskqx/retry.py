...
class ExponentialBackoff:
    ...
    def __post_init__(self) -> None:
        if (
            _seconds(self.initial) < 0
            or _seconds(self.maximum) < 0
    ...
    def delay_for(self, attempt: int) -> timedelta:
        _validate_attempt(attempt)
        seconds = min(
            _seconds(self.maximum),
            _seconds(self.initial) * self.factor ** (attempt - 1),
        )
        return timedelta(seconds=random.uniform(0, seconds) if self.jitter else seconds)
...
class RetryPolicy:
    ...
    @classmethod
    def fixed(
        cls, *, delay: float | timedelta, max_attempts: int = 3, jitter: bool = False
    ) -> RetryPolicy:
        return cls(
            max_attempts=max_attempts, backoff=FixedBackoff(delay, jitter=jitter)
        )
    ...
    def exponential(
        cls,
        *,
        initial_delay: float | timedelta = 1,
        max_delay: float | timedelta = 60,
        factor: float = 2,
        max_attempts: int = 3,
        jitter: bool = False,
    ) -> RetryPolicy:
    ...
    def should_retry(self, error: BaseException) -> bool:
        return not isinstance(error, self.reject_on) and isinstance(
            error, self.retry_on
        )
