...
class TaskWorker:
    ...
    async def start(self) -> None:
        ...
        for index in range(self.concurrency):
            consumer_id = (
                ...
                else f"{self._consumer_id}-{index + 1}"
            )
            consumer = self._broker.consumer(
                self.queue, consumer_id=consumer_id, options=self._options
            )
            await consumer.start()
            self._consumers.append(consumer)
            self._tasks.append(
                asyncio.create_task(
                    self._serve(consumer), name=f"taskqx:{self.queue}:{index}"
                )
            )
    ...
    async def _handle(self, delivery: TaskDelivery) -> None:
        ...
        try:
            ...
            await self._reject(delivery, exc, _reason(exc))
        except PayloadDecodingError as exc:
            await self._reject(delivery, exc, "poison_payload")
        except RetryableError as exc:
            await self._retry(delivery, exc)
        except Exception as exc:  # noqa: BLE001 - handler exceptions require a terminal result.
            ...
            await delivery.ack()
            await metric(
                getattr(self._broker, "metrics", None),
                "processing_duration",
    ...
    async def _retry(self, delivery: TaskDelivery, error: BaseException) -> None:
        ...
        delay = None if policy is None else policy.delay_for(delivery.attempt)
        self._log_failure(
            logging.WARNING,
            "taskqx handler failed; retrying message",
            delivery,
            error,
            action="retry",
            retry_delay=delay,
        ...
        try:
            if policy is None:
                # Preserve the v0.1 Delivery retry(reason=...) contract.
                outcome = await delivery.retry(reason=_reason(error))
            else:
                outcome = await delivery.retry(
                    reason=_reason(error), delay=delay, max_attempts=policy.max_attempts
                )
        except Exception as transition_error:
            self._log_failure(
                logging.ERROR,
                "taskqx retry state transition failed",
                delivery,
                transition_error,
                action="retry",
                outcome="failed",
                handler_error_type=type(error).__name__,
            )
            raise
        if outcome is FinishOutcome.DEAD_LETTERED:
            self._log_failure(
                logging.ERROR,
                "taskqx retry limit exhausted; message dead-lettered",
                delivery,
                error,
                action="retry",
    ...
    async def _reject(
        self, delivery: TaskDelivery, error: BaseException, reason: str
    ) -> None:
        try:
            outcome = await delivery.reject(reason=reason, error=error)
        except Exception as transition_error:
            self._log_failure(
                logging.ERROR,
                "taskqx reject state transition failed",
                delivery,
                transition_error,
                action="reject",
                outcome="failed",
    ...
    def _log_failure(
        ...
        *,
        ...
        """Log a payload-free, correlatable failure with its original traceback."""
        ...
        logger.log(
            ...
            extra={
                "backend": type(self._broker).__name__,
                "namespace": getattr(self._broker, "_namespace", None),
                "queue": task.queue,
                "message_id": task.id,
                ...
                else retry_delay.total_seconds(),
