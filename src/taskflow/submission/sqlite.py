...
class SQLiteSubmissionBackend(Protocol):
    ...
    async def _expire(self, cursor: aiosqlite.Cursor, message_id: str, now: datetime,
...
class SQLiteSubmissionStore:
    ...
    async def _submit_all(self, submissions: list[PreparedSubmission]) -> list[SubmitResult]:
        if not submissions:
            return []
        broker = self._broker
        await broker.start()
        ...
        async with broker._lock:
            ...
            try:
                ...
                for submission in submissions:
                    results.append(await self._submit_in_transaction(cursor, submission))
    ...
    async def _submit_in_transaction(self, cursor: aiosqlite.Cursor, submission: PreparedSubmission) -> SubmitResult:
        ...
        if submission.dedup_key is not None:
            ...
            if existing is not None:
                return SubmitResult(existing["message_id"], False, SubmitDecision.DUPLICATE, existing["message_id"],
                                    dedup_expires_at=_datetime(existing["expires_at"]))
            await cursor.execute("INSERT INTO dedup_records VALUES (?, ?, ?, ?)", (
                submission.dedup_scope, submission.dedup_key, submission.message_id,
        ...
        await cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)
            """, (submission.message_id, submission.queue, submission.envelope, submission.serializer_name,
                  submission.serializer_version, submission.status, submission.max_attempts, _timestamp(now),
        ...
        if submission.status == MessageStatus.EXPIRED.value:
            await broker._expire(cursor, submission.message_id, now, MessageStatus.READY, 0)
