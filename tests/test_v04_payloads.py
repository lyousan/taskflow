...
from taskqx.payloads import decode_payload, normalize_payload
...
async def _wait_for(
    broker: SQLiteBroker, *, acked: int = 0, dead_letters: int = 0
) -> None:
    for _ in range(200):
...
def test_dataclass_and_typeddict_payloads_are_strictly_decoded() -> None:
    ...
    assert decode_payload(
        payload, ResizeImage, schema_name=schema.name, schema_version=schema.version
    ) == ResizeImage("img-1", 800, 600)
    with pytest.raises(PayloadDecodingError):
        decode_payload(
            {"image_id": "img-1", "width": "800", "height": 600},
            ResizePayload,
            schema_name=f"{ResizePayload.__module__}.{ResizePayload.__qualname__}",
            schema_version="1",
        )
...
def test_typeddict_required_and_not_required_fields_are_strict() -> None:
    payload, schema = normalize_payload(
        {
            "image_id": "img-1",
            "width": 800,
            "nested": {"image_id": "nested", "width": 1, "height": 2},
        },
    ...
    assert (
        decode_payload(
            payload,
            OptionalResizePayload,
            schema_name=schema.name,
            schema_version=schema.version,
        )
        == payload
    )
    assert normalize_payload({"image_id": "img-1"}, payload_type=OptionalResizePayload)[
        0
    ] == {"image_id": "img-1"}
    ...
    with pytest.raises(ValidationError):
        normalize_payload(
            {"image_id": "img-1", "width": "800"}, payload_type=OptionalResizePayload
        )
...
def test_pydantic_v2_payload_round_trips_json_values_in_strict_mode() -> None:
    ...
    assert (
        decode_payload(
            payload, Model, schema_name=schema.name, schema_version=schema.version
        )
        == original
    )
    with pytest.raises(PayloadDecodingError):
...
@pytest.mark.asyncio
async def test_typed_worker_receives_dataclass_and_sends_invalid_payload_to_dlq() -> (
    None
):
    ...
    async with SQLiteBroker() as broker:
        ...
        await broker.submit(
            queue="images", payload={"image_id": "img-2", "width": 800, "height": 600}
        )
    ...
    assert dead_letter.message.payload == {
        "image_id": "img-2",
        "width": 800,
        "height": 600,
    }
...
async def test_typeddict_submit_marks_schema_and_reaches_typed_worker() -> None:
    ...
    async with SQLiteBroker() as broker:
        ...
        await broker.submit(
            queue="images",
            payload={"image_id": "typed", "width": 10, "height": 20},
            payload_type=ResizePayload,
        )
        await _wait_for(broker, acked=1)
        await worker.close()
...
@pytest.mark.asyncio
async def test_non_atomic_batch_returns_every_item_and_continues_after_store_failure(
    tmp_path: Path,
) -> None:
    ids = iter(("first", "first", "third"))
    async with SQLiteBroker(
        tmp_path / "batch.db", id_factory=lambda: next(ids)
    ) as broker:
...
@pytest.mark.asyncio
async def test_dead_letter_replay_normalizes_typed_payload_and_replaces_schema() -> (
    None
):
    async with SQLiteBroker() as broker:
        submitted = await broker.submit(
            queue="images", payload=ResizeImage("broken", 1, 2)
        )
    ...
    assert (
        ...
        == f"{ResizePayload.__module__}.{ResizePayload.__qualname__}"
    )
    assert replayed.payload_schema_version == "1"

    async with SQLiteBroker() as broker:
        submitted = await broker.submit(
            queue="images", payload=ResizeImage("typed", 1, 2)
        )
        delivery = await broker.consumer("images").__anext__()
        await delivery.reject(reason="repair")
        await broker.admin.replay_dead_letter(
...
@pytest.mark.asyncio
async def test_dead_letter_replay_preserves_v03_none_payload_semantics_and_can_explicitly_replace_with_null() -> (
    None
):
    async with SQLiteBroker() as broker:
        submitted = await broker.submit(
            queue="images", payload=ResizeImage("kept", 1, 2)
        )
        delivery = await broker.consumer("images").__anext__()
        await delivery.reject(reason="repair")

        await broker.admin.replay_dead_letter(
            "images", submitted.message_id, payload=None, dedup_mode="remove"
        )
        preserved = await broker.inspect_message(submitted.message_id)
        assert preserved is not None
        assert preserved.payload == {"image_id": "kept", "width": 1, "height": 2}
        assert (
            preserved.payload_schema_name
            == f"{ResizeImage.__module__}.{ResizeImage.__qualname__}"
        )
...
def test_replay_dedup_mode_is_explicit_and_compatible() -> None:
    from taskqx.admin import resolve_replay_dedup_mode

    assert (
        resolve_replay_dedup_mode(
            dedup_mode="keep", reuse_dedup=None, has_replacement=False
        )
        == "keep"
    )
    assert (
        resolve_replay_dedup_mode(
            dedup_mode="remove", reuse_dedup=None, has_replacement=False
        )
        == "remove"
    )
    assert (
        resolve_replay_dedup_mode(
            dedup_mode="replace", reuse_dedup=False, has_replacement=True
        )
        == "replace"
    )
    with pytest.raises(ValidationError):
        resolve_replay_dedup_mode(
            dedup_mode="keep", reuse_dedup=False, has_replacement=False
        )
    with pytest.raises(ValidationError):
        resolve_replay_dedup_mode(
            dedup_mode="replace", reuse_dedup=None, has_replacement=False
        )


def test_cli_is_read_only_by_default_and_redacts_payload(
...
def test_cli_redaction_include_payload_and_confirmed_replay(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    from taskqx.cli import main
    ...
    async def seed() -> str:
        async with SQLiteBroker(database) as broker:
            submitted = await broker.submit(
                queue="images", payload={"secret": "keep-private"}
            )
            delivery = await broker.consumer("images").__anext__()
            await delivery.reject(reason="repair")
    ...
    assert (
        main(["--sqlite", str(database), "--json", "message", "inspect", message_id])
        == 0
    )
