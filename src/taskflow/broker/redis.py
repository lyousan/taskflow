...
class RedisBroker:
    ...
    async def list_namespaces(self) -> tuple[str, ...]:
        """Return namespaces discovered from Taskflow schema markers."""
        ...
        while len(items) < limit:
            ...
            for raw_key in keys:
                ...
                if not key.startswith(prefix) or not key.endswith(suffix):
                    continue
                queue = key[len(prefix) : -len(suffix)]
                if queue in seen_queues:
                    continue
                seen_queues.add(queue)
                items.append(await self.observe_queue(queue))
                if len(items) == limit:
                    break
            if scan_cursor == 0:
                break
            ...
            for key in keys:
                ...
                items.append(
                    self._message_summary_from_fields(
                        key.removeprefix(f"{self._namespace}:message:"),
                        fields,
                        message_status,
                    )
                )
