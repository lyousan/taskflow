        ...
        for item in events:
            try:
                message = broker._decode_message(
                    item.envelope, item.serializer_name, item.serializer_version
                )
                ...
                await event(
                    ...
                    serializer_name=item.serializer_name,
                    serializer_version=item.serializer_version,
                )
            except Exception:
                logger.exception(
                    "taskqx failed to publish committed maintenance event",
                    extra={"event": item.name},
                )

    async def maintain(
        self, cursor: aiosqlite.Cursor, now: datetime, queue: str | None = None
    ) -> tuple[int, list[MaintenanceEvent]]:
        """Apply due, expiry, lease-reclaim and ACK-tombstone cleanup atomically."""
        broker = self._broker
        ...
        expired = tuple(
            await (
                await cursor.execute(
                    "SELECT * FROM messages WHERE status IN (?, ?, ?) AND expires_at IS NOT NULL AND expires_at<=?"
                    + predicate,
                    [
        ...
        for row in expired:
            await broker._state_machine.expire(
                cursor, row["id"], now, MessageStatus(row["status"]), row["attempt"]
            )
            events.append(
                self.event(
                    row,
                    ...
                    reason=f"expired_from:{row['status']}",
        ...
        due = tuple(
            await (
                await cursor.execute(
                    "SELECT * FROM messages WHERE status=? AND available_at IS NOT NULL AND available_at<=?"
                    + predicate,
                    [MessageStatus.DELAYED.value, _timestamp(now), *params],
                )
            ).fetchall()
        )
        for row in due:
            await cursor.execute(
                "UPDATE messages SET status=?, available_at=NULL, last_action='due' WHERE id=?",
        ...
        leases = tuple(
            await (
                await cursor.execute(
                    ...
                    [MessageStatus.LEASED.value, _timestamp(now), *params],
                )
            ).fetchall()
        )
        for row in leases:
            if row["attempt"] >= row["max_attempts"]:
                await broker._state_machine.dead_letter(
                    cursor, row, now, "lease_timeout", "租约超时且已达到最大尝试次数"
                )
                events.append(
                    self.event(
                        row,
                        ...
                        reason="租约超时且已达到最大尝试次数",
                        metric_name="dead_lettered_total",
                    )
                )
            else:
                await cursor.execute(
                    "UPDATE messages SET status=?, last_delivery_id=delivery_id, last_consumer_id=consumer_id, consumer_id=NULL, delivery_id=NULL, lease_token=NULL, claimed_at=NULL, lease_until=NULL, last_action='reclaimed', last_reason=? WHERE id=?",
                    (MessageStatus.READY.value, "租约超时", row["id"]),
                )
                await broker._state_machine.counter(
                    cursor, row["queue"], "reclaimed_total"
                )
                events.append(
                    self.event(
                        row,
        ...
        retained = tuple(
            await (
                await cursor.execute(
                    "SELECT id FROM messages WHERE status=? AND retention_until IS NOT NULL AND retention_until<=?"
                    + predicate,
                    [MessageStatus.ACKED.value, _timestamp(now), *params],
                )
            ).fetchall()
        )
        ...
        if retained_ids:
            await cursor.executemany(
                "UPDATE messages SET envelope=X'', payload_pruned=1, retention_until=NULL WHERE id=?",
                retained_ids,
            )
        return len(expired) + len(due) + len(leases) + len(retained_ids), events
