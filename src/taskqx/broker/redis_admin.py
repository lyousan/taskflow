...
class RedisAdmin:
    ...
    async def list_dead_letters(self, queue: str) -> list[DeadLetter]:
        ...
        for message_id in await self._broker._redis.lrange(
            ...
            data = await self._broker._redis.hgetall(
                self._broker._message_key(queue, message_id)
            )
            result.append(
                DeadLetter(
    ...
    async def _page_records(
        ...
        records = [
            await self._broker._redis.hgetall(self._broker._message_key(queue, message_id))
    ...
    async def delete_queue(self, queue: str) -> int:
        ...
        message_ids = {
            _as_text(raw_key).rsplit(":", 1)[-1]
            async for raw_key in self._broker._redis.scan_iter(
                match=f"{self._broker._namespace}:queue:{{{queue}}}:message:*"
            )
        }
        dedup_keys: list[str] = []
        async for raw_key in self._broker._redis.scan_iter(
            ...
            if message_id is not None and _as_text(message_id) in message_ids:
        ...
        await self._broker._redis.srem(self._broker._queue_catalog_key(), queue)
        if message_ids:
            await self._broker._redis.hdel(
                self._broker._message_index_key(), *message_ids
            )
