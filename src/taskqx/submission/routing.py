...
class SubmissionRouter:
    ...
    def __init__(
        ...
        *,
        ...
        configured = dict(
            submission_stores or {"default": submission_store or default_store}
        )
        if "default" not in configured:
            raise ValidationError("submission_stores 必须包含 default profile")
        self._stores = {
            name: store(owner)
            if callable(store) and not hasattr(store, "submit")
            else store
            for name, store in configured.items()
        }
        for name in self._stores:
            validate_persistent_name(
                name, label="submission profile", allow_legacy=owner._allow_legacy_names
            )
        if any(
            not hasattr(store, "submit")
            or not hasattr(store, "submit_many")
            or not hasattr(store, "capabilities")
            for store in self._stores.values()
        ):
            raise ValidationError(
                "每个 submission store 都必须实现 submit 与 submit_many"
            )
        ...
        for queue, profile in self._profiles.items():
            ...
            if profile not in self._stores:
                raise ValidationError(
                    f"queue {queue!r} 使用了未知 submission profile {profile!r}"
                )
    ...
    def for_queue(self, queue: str) -> Any:
        return (
            self.default
            if self.profile_for(queue) == "default"
            else self._stores[self.profile_for(queue)]
        )
