...
class RedisStateMachine:
    ...
    async def claim(self, queue: str, consumer_id: str, lease_seconds: float) -> RedisDelivery | None:
        broker = self._broker
        await broker.maintain(queue)
        await broker._ensure_group(queue)
        now = await broker._now()
        received = await broker._redis.xreadgroup(
            broker._group_name(), consumer_id, {broker._queue_key(queue, "stream"): ">"}, count=1, block=1,
        )
        if not received:
            return None
        ...
        status = int(await claim_call(
            broker, queue=queue, message_id=message_id, now=_timestamp(now), consumer_id=consumer_id,
            delivery_id=delivery_id, token=token, lease_until=_timestamp(lease_until), entry_id=entry_id,
        ).execute(broker._redis))
        if status == -1:
            await broker._observability.maintenance(message_id, "expired", MessageStatus.EXPIRED.value,
                                                     reason="expired_during_claim", metric_name="expired_total")
            return None
        if status <= 0:
            return None
        fields = await broker._redis.hgetall(broker._message_key(message_id))
        delivery = RedisDelivery(
            broker, broker._decode(fields["envelope"], fields.get("serializer_name"), fields.get("serializer_version")),
            delivery_id, token, consumer_id, status, now, lease_until,
        )
        await broker._observability.claimed(delivery)
        return delivery

    async def finish(self, delivery: RedisDelivery, action: str, reason: str | None = None,
        ...
        result = int(await finish_call(
            broker, queue=queue, message_id=message_id, action=action, delivery_id=delivery.delivery_id,
            token=delivery._lease_token, now=_timestamp(now), reason=reason or "", error_type=error_type,
            retry_available_at=_timestamp(now + delay) if delay and delay.total_seconds() > 0 else None,
        ...
        await broker._observability.finished(delivery, outcome, reason=reason, error_type=error_type or None)
        return outcome
    ...
    async def extend(self, delivery: RedisDelivery, seconds: float | None) -> datetime:
        ...
        fields = await broker._redis.hgetall(broker._message_key(delivery.message.id))
        if fields.get("expires_at") and float(fields["expires_at"]) > 0:
            until = min(until, _datetime(float(fields["expires_at"])) or until)
        result = int(await extend_lease_call(
            broker, queue=delivery.message.queue, message_id=delivery.message.id,
            delivery_id=delivery.delivery_id, token=delivery._lease_token, lease_until=_timestamp(until),
        ).execute(broker._redis))
        if result == -1:
            await broker._observability.maintenance(
    ...
    async def recover_uncommitted_pel(self, queue: str) -> int:
        ...
        try:
            _, entries, _ = await broker._redis.xautoclaim(
                stream, broker._group_name(), "taskflow-reclaimer",
                min_idle_time=broker._pending_recovery_ms, start_id="0-0", count=100,
            )
        ...
        for entry_id, fields in entries:
            message_id = fields["message_id"]
            moved = int(await pel_recover_call(
                broker, queue=queue, message_id=message_id, entry_id=entry_id,
            ).execute(broker._redis))
            restored += moved
            if moved:
                await broker._observability.maintenance(
                    message_id, "pel_recovered", MessageStatus.READY.value,
