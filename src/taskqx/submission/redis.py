...
class RedisSubmissionStore:
    ...
    def __init__(self, broker: object, *, namespace: str = "taskqx") -> None:
        self._broker: RedisSubmissionBackend = (
            broker
            if _is_submission_backend(broker)
            else _RedisStoreBackend(broker, namespace)
        )
    ...
    async def submit(self, submission: PreparedSubmission) -> SubmitResult:
        ...
        values = await submit_call(
            self._broker, submission, self._dedup_redis_key(submission)
        ...
        if int(values[0]) == 0:
            ...
            return SubmitResult(
                ...
                str(values[1]),
                dedup_expires_at=submission.created_at
                + timedelta(milliseconds=max(ttl_left, 0)),
            )
        return SubmitResult(
            ...
            dedup_expires_at=(
                submission.created_at + timedelta(milliseconds=submission.dedup_ttl_ms)
    ...
    async def submit_many(
        ...
        values = await batch_submit_call(
            self._broker,
            submissions,
            [self._dedup_redis_key(submission) for submission in submissions],
        ).execute(self._broker._redis)
        results: list[SubmitResult] = []
        for index, submission in enumerate(submissions):
            accepted, message_id, ttl_or_entry, entry = values[
                index * 4 : index * 4 + 4
            ]
            if int(accepted) == 0:
                results.append(
                    SubmitResult(
                        ...
                        str(message_id),
                        dedup_expires_at=submission.created_at
                        + timedelta(milliseconds=max(int(ttl_or_entry), 0)),
                    )
                )
            else:
                results.append(
                    SubmitResult(
                        ...
                        True,
                        SubmitDecision.ACCEPTED,
                        stream_entry_id=str(entry),
                        dedup_expires_at=(
                            submission.created_at
                            + timedelta(milliseconds=submission.dedup_ttl_ms)
                            if submission.dedup_ttl_ms is not None
                            else None
                        ),
                    )
...
class RedisStringDedupSubmissionStore(RedisSubmissionStore):
    ...
    def _dedup_redis_key(self, submission: PreparedSubmission) -> str:
        if submission.dedup_key is None:
            return ""
        assert (
            submission.dedup_scope is not None and submission.dedup_ttl_ms is not None
        )
