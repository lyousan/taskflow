...
class QueueConfig:
    ...
    ack_tombstone_ttl: timedelta | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.ack_tombstone_ttl is not None and (
            not isinstance(self.ack_tombstone_ttl, timedelta)
            or self.ack_tombstone_ttl.total_seconds() <= 0
        ):
            raise ValidationError("QueueConfig.ack_tombstone_ttl 必须为正数或 None")
