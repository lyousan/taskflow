...
# Replay scripts use seven keys.  DLQ ARGV layout: id, envelope, queue, attempt,
# expires_at, now, reuse_dedup, replacement_ttl.  EQ additionally carries new
# expires_at and target queue; see RedisAdmin for the matching call sites.
REPLAY_DEAD_LETTER = """
...
local entry = redis.call('XADD', KEYS[3], '*', 'message_id', ARGV[1], 'envelope', ARGV[2])
redis.call('HSET', KEYS[1], 'envelope', ARGV[2], 'queue', ARGV[7], 'status', 'ready', 'entry_id', entry, 'expires_at', ARGV[3], 'last_action', 'replayed')
...
# KEYS: message, leases, expiry, stream, eq, stats, dlq, ready, delayed.
# ARGV: action, delivery_id, token, now, message_id, reason, error_type, group,
# retry_available_at, requested_max_attempts.
FINISH = """
...
redis.call('HSET', KEYS[2], 'envelope', ARGV[4], 'queue', ARGV[5], 'status', ARGV[6], 'entry_id', entry, 'attempt', '0', 'max_attempts', ARGV[7], 'created_at', ARGV[8], 'expires_at', ARGV[9], 'available_at', ARGV[13], 'serializer_name', ARGV[10], 'serializer_version', ARGV[11])
