...
async def metric(
    metrics: MetricsSink | None, name: str, value: float = 1, **labels: str
) -> None:
    if metrics is None:
        return
    try:
        ...
        logger.exception("taskqx metrics sink failed", extra={"metric": name})
...
async def event(
    ...
    *,
    ...
    delivery_id = (
        getattr(delivery, "delivery_id", None) if delivery is not None else delivery_id
    )
    consumer_id = (
        getattr(delivery, "consumer_id", None) if delivery is not None else consumer_id
    )
    ...
    await middleware.emit(
        ...
        BrokerEvent(
            ...
            item.consumer_id,
            item.reason,
            item.serializer_name,
            item.serializer_version,
            item.error_type,
            item.backend,
        ),
    )
    if event_sink is not None:
        try:
            ...
            logger.exception("taskqx event sink failed", extra={"event": name})
