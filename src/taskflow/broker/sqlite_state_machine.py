...
class SQLiteStateMachine:
    ...
    async def claim(self, queue: str, consumer_id: str, lease_seconds: float) -> SQLiteDelivery | None:
        broker = self._broker
        await broker.start()
        now = broker._now()
        ...
        async with broker._lock:
            ...
            try:
                ...
                row = await (await cursor.execute(
                    ...
                    (queue, MessageStatus.READY.value))).fetchone()
                if row is None:
                    ...
                    await cursor.execute(
                        ...
                        (MessageStatus.LEASED.value, consumer_id, delivery_id, token, _timestamp(now),
                         _timestamp(lease_until), row["id"]))
        ...
        if updated is None:
            return None
        message = broker._decode_message(updated["envelope"], updated["serializer_name"], updated["serializer_version"])
        delivery = SQLiteDelivery(broker, message, delivery_id, token, consumer_id, updated["attempt"], now, lease_until)
        await broker.middleware.emit("after_claim", delivery)
        await metric(broker.metrics, "claimed_total", queue=queue)
        await event(broker.middleware, "claimed", message, status=MessageStatus.LEASED.value, delivery=delivery,
                    backend="sqlite", event_sink=broker.event_sink,
                    serializer_name=broker._serializer.name, serializer_version=broker._serializer.version)
        return delivery
    ...
    async def extend(self, delivery: SQLiteDelivery, seconds: float | None) -> datetime:
        ...
        async with broker._lock:
            ...
            try:
                ...
                if (row is None or row["status"] != MessageStatus.LEASED.value or
                        row["delivery_id"] != delivery.delivery_id or row["lease_token"] != delivery._lease_token or
                        row["lease_until"] <= _timestamp(now)):
                    raise LeaseLostError("租约已经失效，不能续租")
                until = now + timedelta(seconds=period)
                if row["expires_at"] is not None:
                    until = min(until, _datetime(row["expires_at"]) or until)
                if until <= now:
                    await broker._expire(cursor, row["id"], now, MessageStatus.LEASED, row["attempt"])
    ...
    async def finish(self, delivery: SQLiteDelivery, action: str, reason: str | None = None,
        ...
        async with broker._lock:
            ...
            try:
                ...
                if row["expires_at"] is not None and row["expires_at"] <= _timestamp(now):
                    ...
                    await cursor.execute("UPDATE messages SET status=?, last_delivery_id=delivery_id, last_consumer_id=consumer_id, consumer_id=NULL, delivery_id=NULL, lease_token=NULL, claimed_at=NULL, lease_until=NULL, last_action=?, last_reason=NULL WHERE id=?", (MessageStatus.ACKED.value, action, row["id"]))
                    ...
                    limit = min(row["max_attempts"], max_attempts) if max_attempts is not None else row["max_attempts"]
                    if limit < 1:
                        raise ValidationError("max_attempts 必须大于等于 1")
                    if row["attempt"] >= limit:
                        await broker._dead_letter(cursor, row, now, "retry_limit", reason, last_action="retry")
                        outcome = FinishOutcome.DEAD_LETTERED
                    else:
                        available_at = now + delay if delay and delay.total_seconds() > 0 else None
                        await cursor.execute("UPDATE messages SET status=?, available_at=?, last_delivery_id=delivery_id, last_consumer_id=consumer_id, consumer_id=NULL, delivery_id=NULL, lease_token=NULL, claimed_at=NULL, lease_until=NULL, last_action=?, last_reason=? WHERE id=?", ((MessageStatus.DELAYED if available_at else MessageStatus.READY).value, _timestamp(available_at) if available_at else None, action, reason, row["id"]))
                ...
                raise
        event_names = {
            FinishOutcome.ACKED: ("ack", "acked_total"), FinishOutcome.RETRIED: ("retry", "retried_total"),
            FinishOutcome.DEAD_LETTERED: ("dead_lettered", "dead_lettered_total"), FinishOutcome.EXPIRED: ("expired", "expired_total"),
        }
        ...
        await event(broker.middleware, event_name, delivery.message, status=outcome.value, delivery=delivery,
                    reason=reason, error_type=type(error).__name__ if error else None, backend="sqlite",
                    event_sink=broker.event_sink, serializer_name=broker._serializer.name,
    ...
    async def dead_letter(self, cursor: aiosqlite.Cursor, row: aiosqlite.Row, now: datetime,
                          source: str, reason: str | None, error: BaseException | None = None,
        ...
        await cursor.execute("UPDATE messages SET status=?, last_delivery_id=delivery_id, last_consumer_id=consumer_id, consumer_id=NULL, delivery_id=NULL, lease_token=NULL, claimed_at=NULL, lease_until=NULL, last_action=?, last_reason=? WHERE id=?", (MessageStatus.DEAD_LETTERED.value, last_action or source, reason, row["id"]))
        await self.counter(cursor, row["queue"], "dead_lettered_total")

    async def expire(self, cursor: aiosqlite.Cursor, message_id: str, now: datetime,
                     old_status: MessageStatus, attempt: int) -> None:
        ...
        await cursor.execute("UPDATE messages SET status=?, last_delivery_id=delivery_id, last_consumer_id=consumer_id, consumer_id=NULL, delivery_id=NULL, lease_token=NULL, claimed_at=NULL, lease_until=NULL, last_action='expired' WHERE id=?", (MessageStatus.EXPIRED.value, message_id))
