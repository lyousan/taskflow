...
from taskqx import RedisBroker, SQLiteBroker, ValidationError
...
async def broker(
    ...
    namespace = f"taskqx-replay-v04-{uuid4()}"
...
async def _replay_dead_letter(
    broker: Broker, message_id: str, mode: str, *, target_queue: str | None = None
) -> None:
    if mode == "replace":
        ...
        return
    await broker.admin.replay_dead_letter(
        "jobs", message_id, target_queue=target_queue, dedup_mode=mode
    )


async def _replay_expired(
    broker: Broker, message_id: str, mode: str, *, target_queue: str | None = None
) -> None:
    if mode == "replace":
        ...
        return
    await broker.admin.replay_expired(
        "jobs", message_id, expires_at=None, target_queue=target_queue, dedup_mode=mode
    )


@pytest.mark.asyncio
@pytest.mark.parametrize("mode", ("keep", "remove", "replace"))
async def test_dlq_replay_dedup_modes_target_queue_and_repeat(
    broker: Broker, mode: str
) -> None:
    ...
    with pytest.raises(ValidationError):
        await _replay_dead_letter(
            broker, first.message_id, mode, target_queue="repairs"
        )

    original = await broker.submit(
        queue="jobs",
        payload={"id": "old"},
        dedup_scope="original",
        dedup_key="old",
    ...
    if mode == "keep":
        assert (
            not original.accepted and original.existing_message_id == first.message_id
        )
        ...
        assert (
            not replacement.accepted
            and replacement.existing_message_id == first.message_id
        )


@pytest.mark.asyncio
@pytest.mark.parametrize("mode", ("keep", "remove", "replace"))
async def test_eq_replay_dedup_modes_and_target_queue(
    broker: Broker, mode: str
) -> None:
    first = await broker.submit(
        queue="jobs",
    ...
    assert (
        ...
        and replayed.queue == "repairs"
        and replayed.expires_at is None
    )
    with pytest.raises(ValidationError):
...
async def test_replay_dedup_conflict_keeps_original_audit_record(
    broker: Broker, expired: bool
) -> None:
    if expired:
        first = await broker.submit(
            queue="jobs", payload={"id": 1}, expires_at=utc_now() - timedelta(seconds=1)
        )
    else:
        first = await broker.submit(queue="jobs", payload={"id": 1})
    if expired:
        blocker = await broker.submit(
            queue="jobs",
            payload={"id": 2},
            dedup_scope="conflict",
            dedup_key="key",
            dedup_ttl=timedelta(minutes=1),
        )
        with pytest.raises(ValidationError):
        ...
        assert any(
            item.message.id == first.message_id
