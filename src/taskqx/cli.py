"""Taskqx 的安全运维命令行，仅调用公开 Broker/Admin API。"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="taskqx")
    source = parser.add_mutually_exclusive_group()
    source.add_argument("--sqlite", metavar="PATH", default="taskqx.db")
    source.add_argument("--redis-url", metavar="URL")
    parser.add_argument("--namespace", default="taskqx")
    parser.add_argument("--json", action="store_true", dest="as_json")
    parser.add_argument("--include-payload", action="store_true")
    commands = parser.add_subparsers(dest="command", required=True)
    queue = commands.add_parser("queue").add_subparsers(
    ...
    message = commands.add_parser("message").add_subparsers(
    ...
    replay.add_argument(
        "--dedup-mode", choices=("keep", "remove", "replace"), default="keep"
    )
    replay.add_argument("--yes", action="store_true")
    commands.add_parser("health")
    redis = commands.add_parser("redis").add_subparsers(
        dest="redis_command", required=True
    )
    migrate_keyspace = redis.add_parser("migrate-keyspace")
    migrate_keyspace.add_argument("--apply", action="store_true")
    migrate_keyspace.add_argument("--yes", action="store_true")
    for interactive_command in ("tui", "shell"):
        interactive = commands.add_parser(interactive_command)
        interactive_source = interactive.add_mutually_exclusive_group()
        interactive_source.add_argument(
            "--sqlite", metavar="PATH", default=argparse.SUPPRESS
        )
        interactive_source.add_argument(
            "--redis-url", metavar="URL", default=argparse.SUPPRESS
        )
        interactive.add_argument("--namespace", default=argparse.SUPPRESS)
    args = parser.parse_args(argv)
    try:
        return asyncio.run(_run(args))
    except ValidationError as exc:
        print(f"taskqx: error: {exc}", file=sys.stderr)
        return 2
    except Exception as exc:  # noqa: BLE001 - surface backend connection failures cleanly to operators
        print(
            f"taskqx: backend unavailable: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 1
...
async def _run(args: argparse.Namespace) -> int:
    ...
    backend = "redis" if args.redis_url else "sqlite"
    namespace = args.namespace if args.redis_url else None
    context = {"backend": backend, "namespace": namespace}
    if args.command in {"tui", "shell"}:
        return await _run_interactive(
            args, broker, backend=backend, namespace=namespace
        )
    async with broker:
        if args.command == "redis":
            if not args.redis_url:
                raise ValidationError("redis migrate-keyspace 需要 --redis-url")
            assert isinstance(broker, RedisBroker)
            if args.apply and not args.yes:
                raise ValidationError(
                    "迁移 Redis keyspace 会改变持久化消息；请同时传入 --apply --yes"
                )
            return _print(
                await broker.migrate_keyspace(dry_run=not args.apply),
                args,
            )
        ...
        if args.command == "queue":
                ...
                return _print(
                    {
                        **context,
                        "queue": args.queue,
                        "stats": await broker.inspect(args.queue),
                    },
                    args,
                )
                ...
                return _print(
                    {
                        **context,
                        "queue": args.queue,
                        "dead_letters": await broker.admin.list_dead_letters(
                            args.queue
                        ),
                    },
                    args,
            ...
            if args.apply and not args.yes:
                raise ValidationError(
                    "修复一致性会改变持久化索引；请同时传入 --apply --yes"
                )
            return _print(
                await broker.repair_consistency(args.queue, dry_run=not args.apply),
                args,
            )
        ...
        return _print(
            {
                **context,
                "replayed": args.message_id,
                "queue": args.target_queue or args.queue,
            },
            args,
        )
...
async def _run_interactive(
    ...
    *,
    ...
    if not sys.stdin.isatty() or not sys.stdout.isatty():
        print(
            "taskqx: error: 交互式界面需要 TTY；请改用现有 JSON CLI", file=sys.stderr
        )
        return 2
    try:
        if args.command == "tui":
        ...
        return await run_shell(broker, backend=backend, namespace=namespace)
    except ModuleNotFoundError as exc:
        if exc.name in {"rich", "textual", "prompt_toolkit"}:
            print("taskqx: error: 交互式界面需要安装 taskqx[tui]", file=sys.stderr)
            return 2
        raise
    finally:
        await broker.close()
...
def _print(value: Any, args: argparse.Namespace) -> int:
    ...
    if args.as_json:
        ...
        print(
            json.dumps(
                rendered, ensure_ascii=False, default=str, indent=2, sort_keys=True
            )
        )
    return 0
...
def _safe_value(value: Any, *, include_payload: bool) -> Any:
    ...
    if isinstance(value, dict):
        return {
            ...
            for key, item in (
                (key, _safe_value(item, include_payload=include_payload))
