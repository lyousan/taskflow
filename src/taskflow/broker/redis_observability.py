...
class RedisObservability:
    ...
    async def claimed(self, delivery: RedisDelivery) -> None:
        ...
        await emit_event(
            broker.middleware, "claimed", delivery.message, status=MessageStatus.LEASED.value,
            delivery=delivery, backend="redis", event_sink=broker.event_sink,
            serializer_name=broker._serializer.name, serializer_version=broker._serializer.version,
        )

    async def finished(self, delivery: RedisDelivery, outcome: FinishOutcome, *, reason: str | None,
        ...
        await emit_event(
            broker.middleware, event_name, delivery.message, status=outcome.value, delivery=delivery,
            reason=reason, error_type=error_type, backend="redis", event_sink=broker.event_sink,
            serializer_name=broker._serializer.name, serializer_version=broker._serializer.version,
        )

    async def maintenance(self, message_id: str, name: str, status: str, *, reason: str | None = None,
                          error_type: str | None = None, metric_name: str | None = None) -> None:
        ...
        try:
            ...
            if not fields:
                return
            message = broker._decode(fields["envelope"], fields.get("serializer_name"), fields.get("serializer_version"))
            if metric_name is not None:
                await metric(broker.metrics, metric_name, queue=message.queue)
            await emit_event(
                broker.middleware, name, message, status=status,
                ...
                attempt=int(fields.get("attempt", 0)), reason=reason, error_type=error_type,
                backend="redis", event_sink=broker.event_sink,
