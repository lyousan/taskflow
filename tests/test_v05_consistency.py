...
from taskflow.broker.sqlite_migrations import apply_sqlite_migrations
...
async def test_sqlite_consistency_dry_run_and_explicit_repair() -> None:
    async with SQLiteBroker() as broker:
        ...
        await broker._connection.execute("DELETE FROM dead_letters WHERE message_id=?", (submitted.message_id,))
        await broker._connection.commit()

        report = await broker.check_consistency("jobs")
        assert not report.consistent
        assert ("missing_dead_letter", submitted.message_id) in {(issue.name, issue.message_id) for issue in report.issues}
...
def test_cli_consistency_requires_explicit_confirmation(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert main(["--sqlite", str(database), "queue", "repair-consistency", "jobs", "--apply"]) == 2
    assert main(["--sqlite", str(database), "queue", "repair-consistency", "jobs", "--apply", "--yes"]) == 0
...
async def test_sqlite_migration_failure_rolls_back_and_can_resume() -> None:
    async with SQLiteBroker() as broker:
        assert broker._connection is not None
        await broker._connection.execute("UPDATE taskflow_schema SET value='1' WHERE key='version'")
        await broker._connection.execute("DROP TABLE taskflow_schema_migrations")
        await broker._connection.commit()
        ...
        version = await (await broker._connection.execute("SELECT value FROM taskflow_schema WHERE key='version'")).fetchone()
        assert version is not None and version[0] == "1"
        assert await (await broker._connection.execute("SELECT name FROM sqlite_master WHERE name='taskflow_schema_migrations'")).fetchone() is None
