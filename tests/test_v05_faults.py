...
from taskqx import ConsumerOptions, RedisBroker, SQLiteBroker


@pytest.mark.asyncio
async def test_sqlite_simulated_process_interruption_recovers_unacked_lease(
    tmp_path: Path,
) -> None:
    ...
    async with SQLiteBroker(database) as restarted:
        recovered = await restarted.consumer(
...
async def test_redis_health_reports_unavailable_backend() -> None:
    report = await RedisBroker(
        _UnavailableRedis(), namespace="unavailable"
    ).health_check()
...
def test_cli_reports_unavailable_redis_without_traceback(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    from taskqx import cli
    ...
    monkeypatch.setattr(
        cli.RedisBroker, "from_url", lambda *_args, **_kwargs: UnavailableBroker()
    )
    assert cli.main(["--redis-url", "redis://unavailable/2", "health"]) == 1
    assert (
        "backend unavailable: ConnectionError: connection refused"
...
async def redis_broker() -> AsyncGenerator[RedisBroker, None]:
    ...
    namespace = f"taskqx-fault-{uuid4()}"
...
@pytest.mark.asyncio
@pytest.mark.redis
async def test_redis_lua_disconnect_before_and_after_ack_is_observable(
    redis_broker: RedisBroker, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert (
        await redis_broker._redis.hget(
            redis_broker._message_key("jobs", before.message_id), "status"
        )
    ) == "leased"
    ...
    assert (
        await redis_broker._redis.hget(
            redis_broker._message_key("jobs", after.message_id), "status"
        )
    ) == "acked"
