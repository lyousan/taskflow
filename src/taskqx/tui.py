...
try:
    _TASKQX_VERSION = version("taskqx")
except PackageNotFoundError:
    _TASKQX_VERSION = "unknown"
...
def build_tui_app(
    ...
    *,
    ...
    loaded_namespace = getattr(broker, "_namespace", namespace)
    if not isinstance(loaded_namespace, str):
    ...
    class OperationsApp(App[None]):
        ...
        def __init__(self) -> None:
            super().__init__()
            self.theme = "dracula"
            self._namespace = loaded_namespace
        ...
        def compose(self) -> ComposeResult:
            with Vertical(id="dashboard"):
                with Horizontal(id="header"):
                    yield Static(
                        f"[bold]TASKQX[/] · {_connection_label(backend, self._namespace)}",
                        id="statusbar",
                    )
                    yield Static("", id="activity", classes="hidden")
                    yield Static(f"v{_TASKQX_VERSION}", id="version")
        ...
        async def _load_namespaces(self) -> None:
            ...
            if selected in available:
                namespace_list.index = available.index(selected)
        ...
        async def on_list_view_selected(self, event: Any) -> None:
            ...
            if selected != self._namespace:
                ...
                self._record_page_index = 0
                self.query_one("#queues", DataTable).clear()
            ...
            self._activated_views.add("queues")
            self.action_refresh(load_queues=True)
        ...
        def _detail(
            ...
            *,
            ...
            payload = (
                ...
                else "<permanently pruned>"
                if getattr(item, "payload_pruned", False)
                else "<redacted>"
            )
            reason = (
            ...
            lineage = " · ".join(
                part
                for part in (
                    f"workflow: {workflow_id}" if workflow_id else None,
                    f"parent: {parent_id}" if parent_id else None,
                )
                if part is not None
            ) or "—"
            acked_at = getattr(item, "acked_at", None)
            acknowledged = acked_at.isoformat() if acked_at is not None else "—"
            return Text.assemble(
                ("消息", "bold"),
                f" {message_id}\n状态: {status_value}  · 尝试: {item.attempt}  · 原因: {reason}\n"
                f"创建: {created_at.isoformat()}  · 确认: {acknowledged}\n"
                f"serializer: {serializer_name}@{serializer_version}  · lineage: {lineage}\n"
                f"payload: {payload}",
            )
        ...
        async def _show_payload(self, selected: tuple[str, Any]) -> None:
            ...
            if message is None:
                if getattr(selected[1], "payload_pruned", False):
                    self.notify("业务内容已按 ACK 保留期永久清除。", severity="warning")
                else:
        ...
        def _select_queue(self, event: Any) -> None:
        ...
        def _update_record_detail(self) -> None:
            selected = self._selected()
        ...
        def on_data_table_row_selected(self, event: Any) -> None:
            if event.data_table.id == "queues":
                self._select_queue(event)
            elif event.data_table.id == "records":
                self._update_record_detail()
