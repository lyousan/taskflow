"""面向本地开发与 CI 的 SQLite Taskqx backend。"""
...
class SQLiteBroker:
    ...
    async def _initialize(self) -> None:
        ...
        await self._connection.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS messages (
                ...
                last_action TEXT, last_reason TEXT, acked_at REAL, retention_until REAL,
                workflow_id TEXT, parent_id TEXT, payload_pruned INTEGER NOT NULL DEFAULT 0
            );
            ...
            CREATE TABLE IF NOT EXISTS taskqx_schema (
        ...
        if "acked_at" not in columns:
            await self._connection.execute(
                "ALTER TABLE messages ADD COLUMN acked_at REAL"
            )
        if "retention_until" not in columns:
            await self._connection.execute(
                "ALTER TABLE messages ADD COLUMN retention_until REAL"
            )
        if "workflow_id" not in columns:
            await self._connection.execute("ALTER TABLE messages ADD COLUMN workflow_id TEXT")
        if "parent_id" not in columns:
            await self._connection.execute("ALTER TABLE messages ADD COLUMN parent_id TEXT")
        if "payload_pruned" not in columns:
            await self._connection.execute(
                "ALTER TABLE messages ADD COLUMN payload_pruned INTEGER NOT NULL DEFAULT 0"
            )
        await self._connection.execute(
            "CREATE INDEX IF NOT EXISTS idx_messages_retention "
            "ON messages(queue, status, retention_until)"
        )
    ...
    async def _scheduler_queues(self) -> tuple[str, ...]:
        """Discover every queue with persisted message state."""
        ...
        return tuple(row["queue"] for row in rows)
    ...
    async def list_message_summaries(
        ...
        *,
        ...
        columns = (
            ...
            "acked_at, workflow_id, parent_id, payload_pruned"
        )
        async with self._read_connection() as connection:
            rows = list(
                await (
    ...
    def _message_summary(row: aiosqlite.Row) -> MessageSummary:
        return MessageSummary(
            ...
            acked_at=_datetime(row["acked_at"]) if row["acked_at"] is not None else None,
