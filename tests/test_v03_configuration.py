...
from taskqx.naming import validate_persistent_name
...
async def test_queue_config_applies_lease_attempt_and_payload_limit() -> None:
    async with SQLiteBroker(
        queues={
            "mail": QueueConfig(
                max_attempts=7, lease=timedelta(seconds=2), max_payload_bytes=8
            )
        },
    ) as broker:
        ...
        row = await (
            await broker._connection.execute(
                "SELECT max_attempts FROM messages WHERE id=?", (result.message_id,)
            )
...
async def test_queue_config_none_overrides_broker_dedup_default() -> None:
    async with SQLiteBroker(
        default_dedup_ttl=timedelta(hours=1),
        queues={"plain": QueueConfig(default_dedup_ttl=None)},
    ) as broker:
        with pytest.raises(ValidationError, match="dedup_ttl"):
            await broker.submit(
                queue="plain", payload={}, dedup_scope="s", dedup_key="k"
            )
...
async def test_queue_config_default_dedup_ttl_is_used() -> None:
    async with SQLiteBroker(
        queues={"dedup": QueueConfig(default_dedup_ttl=timedelta(minutes=1))}
    ) as broker:
        first = await broker.submit(
            queue="dedup", payload={}, dedup_scope="scope", dedup_key="key"
        )
...
async def test_worker_arguments_override_queue_config() -> None:
    configured = QueueConfig(
        lease=timedelta(seconds=7),
        retry_policy=RetryPolicy(max_attempts=5, backoff=FixedBackoff(1)),
    )
    ...
    async with SQLiteBroker(queues={"jobs": configured}) as broker:
        worker = broker.worker(
            "jobs",
            lambda _: None,
            options=ConsumerOptions(lease_seconds=3),
...
async def test_explicit_queue_retry_policy_remains_a_stricter_worker_limit() -> None:
    ...
    async with SQLiteBroker(
        queues={
            "jobs": QueueConfig(
                max_attempts=7, retry_policy=RetryPolicy(max_attempts=2)
            )
        }
    ) as broker:
        worker = broker.worker("jobs", handler)
...
async def test_event_sink_receives_standard_fields() -> None:
    ...
    assert all(
        item.backend == "sqlite" and item.queue == "jobs" for item in sink.events
    )
...
async def test_atomic_batch_reports_duplicate_through_submission_observer() -> None:
    ...
    assert [item.event_name for item in sink.events] == [
        "submitted",
        "duplicate",
        "submitted",
    ]
...
async def test_maintenance_events_are_published_after_state_commit() -> None:
    ...
    assert any(
        item.event_name == "dead_lettered" and item.reason for item in sink.events
    )
...
def test_broker_event_constructor_remains_v02_compatible() -> None:
    event = BrokerEvent(
        name="submitted", timestamp=utc_now(), queue="jobs", message_id="id"
    )
