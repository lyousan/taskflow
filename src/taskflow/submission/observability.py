...
class SubmissionObserver:
    ...
    def __init__(self, *, backend: str, middleware: Middleware,
                 metrics: MetricsSink | None, event_sink: EventSink | None,
    ...
    async def record(self, prepared: PreparedSubmission, message: TaskMessage,
        ...
        if not result.accepted:
            ...
            await event(self._middleware, "duplicate", message, status=prepared.status,
                        backend=self._backend, event_sink=self._event_sink,
        ...
        await event(self._middleware, "submitted", message, status=prepared.status,
                    backend=self._backend, event_sink=self._event_sink,
                    serializer_name=self._serializer.name,
                    serializer_version=self._serializer.version)
        if prepared.status == MessageStatus.EXPIRED.value:
            await metric(self._metrics, "expired_total", queue=message.queue)
            await event(self._middleware, "expired", message,
                        status=MessageStatus.EXPIRED.value,
                        reason="expired_on_submit", backend=self._backend,
