...
class SubmissionService:
    ...
    async def submit(self, **kwargs: Any) -> SubmitResult:
        ...
        result = await self._owner._submission_store_for(prepared.queue).submit(prepared)
        await self._owner._record_submitted(prepared, message, result)
        return result

    async def submit_many(self, messages: list[SubmitRequest], *, atomic: bool) -> list[SubmitResult] | list[BatchSubmitItemResult]:
        if not atomic:
        ...
        prepared_messages = [await self._prepare(**_request_kwargs(request)) for request in messages]
        for _, message in prepared_messages:
            await self._owner.middleware.emit("before_submit", message)
        profiles = {self._owner._queue_submission_profiles.get(prepared.queue, "default")
        ...
        if prepared_messages:
            ...
            atomic_results = await store.submit_many([prepared for prepared, _ in prepared_messages])
        for result, (prepared, message) in zip(atomic_results, prepared_messages, strict=True):
            await self._owner._record_submitted(prepared, message, result)
...
def _request_kwargs(request: SubmitRequest) -> dict[str, Any]:
    return {
        "queue": request.queue, "payload": request.payload, "metadata": request.metadata,
        "dedup_key": request.dedup_key, "dedup_scope": request.dedup_scope,
        "dedup_ttl": request.dedup_ttl, "delay": request.delay, "expires_at": request.expires_at,
        "max_attempts": request.max_attempts, "workflow_id": request.workflow_id,
        "parent_id": request.parent_id, "payload_type": request.payload_type,
    }
