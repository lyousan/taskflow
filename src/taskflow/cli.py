...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"taskflow: backend unavailable: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 1
...
async def _run_interactive(
    ...
    *,
    ...
    if not sys.stdin.isatty() or not sys.stdout.isatty():
        print(
            "taskflow: error: 交互式界面需要 TTY；请改用现有 JSON CLI", file=sys.stderr
        )
        return 2
    try:
        if args.command == "tui":
        ...
        if exc.name in {"rich", "textual", "prompt_toolkit"}:
            print("taskflow: error: 交互式界面需要安装 taskflow[tui]", file=sys.stderr)
            return 2
        raise
    finally:
        await broker.close()
