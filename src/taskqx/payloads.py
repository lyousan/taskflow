...
def schema_for(payload_type: type[Any]) -> PayloadSchema:
    ...
    name = getattr(payload_type, "__taskqx_schema_name__", None)
    version = getattr(payload_type, "__taskqx_schema_version__", "1")
...
def normalize_payload(
    payload: Any, *, payload_type: type[Any] | None = None
) -> tuple[Any, PayloadSchema | None]:
...
def _decode_value(value: Any, annotation: Any) -> Any:
    ...
    if origin is dict:
        ...
        return {
            _decode_value(key, key_type): _decode_value(item, item_type)
    ...
    if is_dataclass(annotation) and isinstance(annotation, type):
        ...
        for field in fields(annotation):
            ...
            decoded[field.name] = _decode_value(
                value[field.name], hints.get(field.name, Any)
            )
    ...
    if is_typeddict(annotation):
        ...
        if unexpected or missing:
            raise PayloadDecodingError(
                f"TypedDict 字段不匹配：缺少 {sorted(missing)!r}，未知 {sorted(unexpected)!r}"
            )
    ...
    if isinstance(annotation, type) and type(value) is annotation:
        return value
    raise PayloadDecodingError(
        f"payload 类型错误：期望 {annotation!r}，收到 {type(value)!r}"
    )
