...
from .redis_calls import due_delayed_call, expire_call, reclaim_lease_call
...
class RedisMaintenance:
    ...
    async def run(self, queue: str | None = None) -> int:
        ...
        for message_id in await broker._redis.zrangebyscore(broker._queue_key(queue, "delayed"), "-inf", _timestamp(now)):
            moved = int(await due_delayed_call(
                broker, queue=queue, message_id=message_id, now=_timestamp(now),
            ).execute(broker._redis))
            reclaimed += int(bool(moved))
            if moved == 1:
                await broker._emit_maintenance_event(message_id, "due", MessageStatus.READY.value)
            elif moved == 2:
                await broker._emit_maintenance_event(message_id, "expired", MessageStatus.EXPIRED.value,
                    reason="expired_from:delayed", metric_name="expired_total")
        for message_id in await broker._redis.zrangebyscore(broker._queue_key(queue, "expiry"), "-inf", _timestamp(now)):
            moved = int(await expire_call(
                broker, queue=queue, message_id=message_id, now=_timestamp(now),
            ).execute(broker._redis))
            if moved:
                await broker._emit_maintenance_event(message_id, "expired", MessageStatus.EXPIRED.value,
                    reason="expired_by_maintenance", metric_name="expired_total")
        for message_id in await broker._redis.zrangebyscore(broker._queue_key(queue, "leases"), "-inf", _timestamp(now)):
            result = int(await reclaim_lease_call(
                broker, queue=queue, message_id=message_id, now=_timestamp(now),
            ).execute(broker._redis))
            reclaimed += int(bool(result))
            transitions = {
                1: ("reclaimed", MessageStatus.READY.value, "reclaimed_total"),
                2: ("dead_lettered", MessageStatus.DEAD_LETTERED.value, "dead_lettered_total"),
            ...
            if result in transitions:
                ...
                await broker._emit_maintenance_event(message_id, name, status, reason=reason, metric_name=metric_name)
        return reclaimed
